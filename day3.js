...
let addPositions = (path, direction, distance) => {
    let p = [...path];
    if (direction === 'U') {
        for (let i = 0; i < distance; i++) {
            p.push([p[p.length - 1][0], p[p.length - 1][1] + 1]);
        }
    } else if (direction === 'R') {
        for (let i = 0; i < distance; i++) {
            p.push([p[p.length - 1][0] + 1, p[p.length - 1][1]]);
        }
    } else if (direction === 'D') {
        for (let i = 0; i < distance; i++) {
            p.push([p[p.length - 1][0], p[p.length - 1][1] - 1]);
...
let wirePositionsList = input => {
    let wires = input.split('\n');

    for (let i = 0; i < wires.length; i++) {
        wires[i] = wires[i].split(',');
        wires[i] = wirePositions(wires[i]);
    }
    return wires;
}

//console.log('La liste des positions des câbles est la suivante : ')
//console.log(wirePositionsList(input));
...
//console.log('La liste des distances est la suivante : ')
//console.log(manhattanDistances(intersectionsList(input)));

let manhattanDistanceToClosestIntersection = input => {
    let intersections = intersectionsList(input);
    let distances = manhattanDistances(intersections);
