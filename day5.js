...
let decode = string => {
    ...
    //the mode of the 1st parameter
    //the mode of the 2nd parameter
    //the mode of the 3rd parameter
    return [s[3] + s[4], s[2], s[1], s[0]];
}

let codeIncrement = string => {
    if (string === '01' || string === '02') {
        return 4;
    } else if (string === '03' || string === '04') {
        return 2;
    } else if (string === '99') {
        return 0;
    } else {
...
let runProgram = input => {
    ...
    let v = (i, j) => {
        if (decode(l[i])[j] === '0') {
            return Number(l[Number(l[i + j])]);
        } else {
            return Number(l[i + j]);
        }
    }
    
    let instructionValue = decode(l[0])[0];
    let increment = codeIncrement(instructionValue);
    let result = [];

    for (let i = 0; i < l.length; i+=increment) {
        console.log('i = ' + i);
        if (decode(l[i])[0] === '01') {
            l[l[i + 3]] = (v(i, 1) + v(i, 2)) + '';
            increment = 4;
        } else if (decode(l[i])[0] === '02') {
            l[l[i + 3]] = v(i, 1) * v(i, 2) + '';
            increment = 4;
        } else if (decode(l[i])[0] === '03') {
            l[l[i + 1]] = 1;
            increment = 2;
        }  else if (decode(l[i])[0] === '04') {
            result.push(v(i, 1));
            increment = 2;
        }  else if (decode(l[i])[0] === '99') {
            i = l.length;
        } else {
